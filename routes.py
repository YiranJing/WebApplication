...
def showhistory():
    ...
    history = database.used_history(user_details['empid'])
    ...
    return(render_template('used_history.html',
                           history=history,
                           session=session,
                           page=page))
...
#   Show Model Details
#####################################################
@app.route('/modeldetail/<string:manufacturer>,<string:modelNumber>')
def modeldetail(manufacturer,modelNumber):
    """
    Shows a list of all models.
    """
    # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))
    models = database.show_model_detail(manufacturer,modelNumber)
...
#   Model Cost
#####################################################
@app.route('/modelcost/<string:model>,<string:manufacturer>')
def modelcost(model,manufacturer):
    # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))

    cost = database.get_model_cost(manufacturer,model)

    if cost is None:
        flash('No model/manufacturer matching')
        page['bar'] = True
        return redirect(url_for('models'))

    return (render_template('model_cost.html',
                            model=model,
...
def add_model():
    ...
    if(request.method == 'POST'):
        ...
        maxNumber = request.form.get('maxNumber')

        if manufacturer is None or modelNumber is None or maxNumber is None:
            page['bar'] = False
            flash('Invalid request')
            return redirect(url_for('add_model'))

        # If it is a POST - they are sending an 'issue' request
        res = database.add_model(session['manager'], manufacturer,modelNumber,description,weight,maxNumber)
        if res == None:
            page['bar'] = True
            flash('Model successfully added')
        else:
            page['bar'] = False
            flash("Manufacturer and model number have already existed.")
        return redirect(url_for('add_model'))
